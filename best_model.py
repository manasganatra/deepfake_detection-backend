...
# =====================================================
# CONFIGURATION - UPDATED FOR 3 SEPARATE DATASETS
# =====================================================

class Config:
    """Configuration for separate train/test/validation datasets"""
    # Dataset paths - UPDATE THESE TO YOUR DATASET STRUCTURE
    TRAIN_REAL_PATH = "C:\\Users\\kg060\\Desktop\\projects\\deepfake-detection\\backend\\dataset\\Dataset\\Train\\Real"
    TRAIN_FAKE_PATH = "C:\\Users\\kg060\\Desktop\\projects\\deepfake-detection\\backend\\dataset\\Dataset\\Train\\Fake"

    VAL_REAL_PATH = "C:\\Users\\kg060\\Desktop\\projects\\deepfake-detection\\backend\\dataset\\Dataset\\Validation\\Real"
    VAL_FAKE_PATH = "C:\\Users\\kg060\\Desktop\\projects\\deepfake-detection\\backend\\dataset\\Dataset\\Validation\\Fake"

    TEST_REAL_PATH = "C:\\Users\\kg060\\Desktop\\projects\\deepfake-detection\\backend\\dataset\\Dataset\\Test\\Real"
    TEST_FAKE_PATH = "C:\\Users\\kg060\\Desktop\\projects\\deepfake-detection\\backend\\dataset\\Dataset\\Test\\Fake"
...
class DeepfakeDataset(Dataset):
    ...
    def __init__(self, real_dir, fake_dir, transform=None, dataset_name="Dataset"):
        self.data = []
        ...
        if os.path.exists(real_dir):
            ...
            print(f"✅ Loaded {len(real_files)} REAL images from {dataset_name}")
        else:
        ...
        if os.path.exists(fake_dir):
            ...
            print(f"✅ Loaded {len(fake_files)} FAKE images from {dataset_name}")
        else:
            print(f"⚠️ Fake directory not found: {fake_dir}")
        
        if len(self.data) == 0:
            raise ValueError(f"❌ No valid images found in {dataset_name}! Check your dataset paths.")
        
        # Shuffle for better training (only for training set)
        if "train" in dataset_name.lower():
            random.shuffle(self.data)
        
        print(f"📊 {dataset_name} size: {len(self.data)} images")
...
# =====================================================
# DATA LOADING - UPDATED FOR SEPARATE DATASETS
# =====================================================

def create_data_loaders():
    """Create data loaders for separate train/val/test datasets"""
    
    print("🔄 Creating data loaders for separate datasets...")
    
    # Validate all paths exist
    paths_to_check = [
        (Config.TRAIN_REAL_PATH, "Training Real"),
        (Config.TRAIN_FAKE_PATH, "Training Fake"),
        (Config.VAL_REAL_PATH, "Validation Real"),
        (Config.VAL_FAKE_PATH, "Validation Fake"),
        (Config.TEST_REAL_PATH, "Test Real"),
        (Config.TEST_FAKE_PATH, "Test Fake")
    ]
    
    for path, name in paths_to_check:
        if not os.path.exists(path):
            print(f"❌ {name} path not found: {path}")
            raise FileNotFoundError(f"Please check your {name} dataset path")
    
    # Create datasets
    train_dataset = DeepfakeDataset(
        Config.TRAIN_REAL_PATH, Config.TRAIN_FAKE_PATH, 
        transform=train_transform, dataset_name="Training Set"
    )
    
    val_dataset = DeepfakeDataset(
        Config.VAL_REAL_PATH, Config.VAL_FAKE_PATH, 
        transform=val_test_transform, dataset_name="Validation Set"
    )
    
    test_dataset = DeepfakeDataset(
        Config.TEST_REAL_PATH, Config.TEST_FAKE_PATH, 
        transform=val_test_transform, dataset_name="Test Set"
    )
    
    # Calculate class weights from training set
    fake_count = sum(1 for _, label in train_dataset.data if label == 0)
    real_count = sum(1 for _, label in train_dataset.data if label == 1)
...
# =====================================================
# MAIN EXECUTION - UPDATED FOR 3 DATASETS
# =====================================================
...
def main():
    ...
    print("📁 Using Separate Train/Validation/Test Datasets")
    print("="*80)
    
    try:
        # Create data loaders for all three datasets
        ...
        train_accuracy = evaluate_model(model, train_loader, "Training Set")
        val_accuracy = evaluate_model(model, val_loader, "Validation Set")
        test_accuracy = evaluate_model(model, test_loader, "Test Set")
        
        # Save model
        save_model(model, optimizer, accuracy=test_accuracy)
